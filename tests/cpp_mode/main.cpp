...
static constexpr bool medianReference = true;
static constexpr bool averageReference = false;
...
static constexpr unsigned int expectCnt[] = {
    0x000000, 0x0018B8, 0x0031F8, 0x004BE3, 0x006511, 0x007E4E, 0x00974C, 0x00B110, 0x00CA2F, 0x00E338,
    0x00FC0E, 0x011556, 0x012E30, 0x014781, 0x01615C, 0x017AD0, 0x0193C4, 0x01AD02, 0x01C636, 0x01DF63,
    0x01F879, 0x0211B7, 0x022ADD, 0x024382, 0x025D1D, 0x027621, 0x028F4D, 0x02A880, 0x02C19B, 0x02DB42,
    0x02F462, 0x030D3C, 0x0325DB, 0x033ED7, 0x0357BE, 0x037084, 0x038967, 0x03A298, 0x03BC00, 0x03D4E4,
    0x03EDAB, 0x04068D, 0x041FAC, 0x043849, 0x0451D5, 0x046AFF, 0x04847E, 0x049D51, 0x04B698, 0x04CFAC,
    0x04E8BD, 0x05024A, 0x051AFF, 0x05340C, 0x054D27, 0x056690, 0x057FB7, 0x05993B, 0x05B276, 0x05CB28,
    0x05E3EA, 0x05FC9D, 0x061614, 0x062EE9, 0x0648AD, 0x06625B, 0x067B52, 0x06947A, 0x06AD24, 0x06C68C,
    0x06DF3D, 0x06F80F, 0x071112, 0x072A30, 0x074366, 0x075D01, 0x07763B, 0x078F1C, 0x07A74A, 0x07BFDF,
    0x07D901, 0x07F1F1, 0x080B29, 0x0823DC, 0x083D5B, 0x0856A6, 0x086F5A, 0x0888DE, 0x08A1CF, 0x08BAD9,
    0x08D416, 0x08EDA9, 0x090685, 0x091F84, 0x0938EA, 0x0951C4, 0x096AED, 0x096AED, 0x096AED, 0x096AED,
    0x096AED};
...
int main(int argc, const char **argv)
{
    ...
    float *buffer = new float[(chunkSize + chunkLeftMargin) * numChannels];
    fprintf(stderr, "address of buffer: %p\n", buffer);
    fprintf(stderr, "\n");
    ...
    for (int i = 0; i < repeat; i++)
    {
        ...
        {
            // long long is needed for *numChannels
            long long left = j * chunkSize - chunkLeftMargin;
            long long right = j * chunkSize + chunkSize;
            long long t0 = 0;
            if (left < 0)
            {
                t0 = -left * numChannels;
            ...
            {
                fprintf(stderr, "chunk: %4d/%4d, frame %8lld~%8lld\n", j, numChunks, left, right);
            }

            dataFile.seekg(dataOffset + left * numChannels * sizeof(float))
                .read((char *)(buffer + t0), (right - left) * numChannels * sizeof(float));
            if (!dataFile.good())
            {
                fprintf(stderr, "ERROR in file reading at chunk %4d/%4d! fail %d, bad %d, eof %d\n",
                        j, numChunks, dataFile.fail(), dataFile.bad(), dataFile.eof());
            }

            pDet->step(buffer, j * chunkSize, chunkSize);
        }
